...
async function checkPresence() {
  try {
    ...
    if (emails.length !== uniqueEmails.size) {
      console.log('  [WARN] DUPLICATE EMAILS FOUND!');
      const duplicates = emails.filter((email, index) => emails.indexOf(email) !== index);
      console.log('  Duplicate emails:', [...new Set(duplicates)]);
    } else {
      console.log('  [OK] All emails are unique');
    }

    if (usernames.length !== uniqueUsernames.size) {
      console.log('  [WARN] DUPLICATE USERNAMES FOUND!');
      ...
      console.log('  [OK] All usernames are unique');
    }
  } catch (error) {
